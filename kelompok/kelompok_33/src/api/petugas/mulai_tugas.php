<?php
// api/petugas/mulai_tugas.php - Petugas mulai mengerjakan tugas
...
try {
    ...
    if (!$penugasan) {
        throw new Exception('Penugasan tidak ditemukan atau belum diterima');
    }
    
    // Update status
    $stmt = $pdo->prepare("
        UPDATE penugasan 
        SET status_penugasan = 'sedang_dikerjakan', started_at = NOW() 
        WHERE id = :id
    ");
